...
let num =
  take_while1 (function
    | '\x20' | '\x0a' | '\x0d' | '\x09' | '[' | ']' | '{' | '}' | ':' | ',' ->
        false
    | _ -> true)
  >>= fun s -> try return (Number (float_of_string s)) with _ -> fail "number"
...
module S = struct
  ...
  let utf_8 buf d = function
    | [ c; b; a ] ->
        ...
        if a lor b lor c lor d = 255 then `Error "invalid hex escape"
        else
          let cp = (a lsl 12) lor (b lsl 8) lor (c lsl 4) lor d in
          if cp >= 0xd800 && cp <= 0xdbff then `UTF16 (cp, `S)
          else (
            Buffer.add_char buf
